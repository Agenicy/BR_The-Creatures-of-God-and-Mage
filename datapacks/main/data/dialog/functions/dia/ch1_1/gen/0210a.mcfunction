...
tellraw @a[scores={st=2}] {"text":"→［算了］繼續遊戲並存檔","color":"aqua","clickEvent":{"action":"run_command","value":"/execute store success score @s dia_st run trigger dia_choose set 50"}}
tellraw @a[scores={st=2}] {"text":"→［確認捨棄］重新開始這一段落（上個存檔點：反抗軍基地外）","color":"aqua","clickEvent":{"action":"run_command","value":"/execute store success score @s dia_st run trigger dia_choose set "}}
