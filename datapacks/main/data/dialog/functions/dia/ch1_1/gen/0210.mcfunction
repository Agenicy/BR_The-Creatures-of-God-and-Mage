...
tellraw @a[scores={st=6}] {"text":"<格藍> 說的也是，德雅並沒有操縱的權能吧。","clickEvent":{"action":"run_command","value":"/function dialog:dia/add"}}
...
tellraw @a[scores={st=9}] {"text":"<卡司> 這……我可能也無法回答。她用來操控赫斯克的並不是神術或是德雅的權能，至少不是一般祭司能做到的程度。","clickEvent":{"action":"run_command","value":"/function dialog:dia/add"}}
...
tellraw @a[scores={st=11}] {"text":"<卡司> 這要從基礎的的原理說起：首先，德雅的權能是共感，因此祂的子民能大略感受到彼此的想法，減少許多不必要的衝突。","clickEvent":{"action":"run_command","value":"/function dialog:dia/add"}}
tellraw @a[scores={st=12}] {"text":"<卡司> 而赫斯克——如同出發前跟你們講過的——是德雅神力的產物，萊歐卡於是利用這一點建立了「萊歐克訊息網路」。","clickEvent":{"action":"run_command","value":"/function dialog:dia/add"}}
tellraw @a[scores={st=13}] {"text":"<卡司> 而芙娃……則有可能是德雅的神選者。這樣一來，她就是等同於德雅的存在，自然可以控制所有德雅神力的創造物。","clickEvent":{"action":"run_command","value":"/function dialog:dia/add"}}
tellraw @a[scores={st=14}] {"text":"<貝露> 神——！（小聲）咳、神選者？","clickEvent":{"action":"run_command","value":"/function dialog:dia/add"}}
tellraw @a[scores={st=15}] {"text":"<格藍> 被德雅親自選中的人嗎……難怪她身上的神力濃度這麼高。","clickEvent":{"action":"run_command","value":"/function dialog:dia/add"}}
tellraw @a[scores={st=16}] {"text":"<亞加> 等一下，親自選中？但是德雅不是——","clickEvent":{"action":"run_command","value":"/function dialog:dia/add"}}
tellraw @a[scores={st=17}] {"text":"<卡司> 對，這也是我們所懷疑的點。自從格蘭德城滅亡、我們的大祭司與德雅神器被萊歐卡奪走後，德雅的本體早就陷入沉睡狀態——這可是六十幾年前的事情，別說芙娃，連我都還沒出生呢。","clickEvent":{"action":"run_command","value":"/function dialog:dia/add"}}
tellraw @a[scores={st=18}] {"text":"<卡司> 既然如此，芙娃應該不會是德雅的神選者才對。但是種種的跡象卻都指向這個可能。","clickEvent":{"action":"run_command","value":"/function dialog:dia/add"}}
