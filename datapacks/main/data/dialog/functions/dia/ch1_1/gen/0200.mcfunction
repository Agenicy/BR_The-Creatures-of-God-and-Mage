...
tellraw @a[scores={st=4}] {"text":"<卡司> 請說？我也挺好奇菲亞爾的祭司是用什麼觀點在看著我們這些格奈拉追隨者的。","clickEvent":{"action":"run_command","value":"/function dialog:dia/add"}}
...
tellraw @a[scores={st=10}] {"text":"<卡司> 招呼語雖然普通，但卻能說明菲亞爾與格奈拉的差別。我們不相信發生壞事後一定會有好事，甚至可能會一直壞下去也說不定；相反的，我們相信的是團結的力量。","clickEvent":{"action":"run_command","value":"/function dialog:dia/add"}}
tellraw @a[scores={st=11}] {"text":"<卡司> 共感與星光的格奈拉，教義是「互助合作、永不放棄」。而這也是我們格蘭德居民的共同信仰。","clickEvent":{"action":"run_command","value":"/function dialog:dia/add"}}
tellraw @a[scores={st=12}] {"text":"<卡司> 你們也看到天空中閃爍的星光了吧？星光是格奈拉的化身，而我們，格奈拉的追隨者、格蘭德反抗軍的所有成員，就是那些星星。雖然星星無法像太陽一樣帶來光明，但若沒了星星，世界就會陷入全然的黑暗。","clickEvent":{"action":"run_command","value":"/function dialog:dia/add"}}
