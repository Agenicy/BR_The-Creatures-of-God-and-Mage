...
tellraw @a[scores={st=4}] {"text":"<格藍> 見證過大戰又如何？難道因為是古蹟就可以阻擋我們的去路嗎？","clickEvent":{"action":"run_command","value":"/function dialog:dia/add"}}
tellraw @a[scores={st=5}] {"text":"<卡司> 話當然不是這麼說的。只是我們該怎麼通過這個柵欄呢？","clickEvent":{"action":"run_command","value":"/function dialog:dia/add"}}
tellraw @a[scores={st=6}] {"text":"<亞加> 我想，或許我們可以……","clickEvent":{"action":"run_command","value":"/function dialog:dia/add"}}
