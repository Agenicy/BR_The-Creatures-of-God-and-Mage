...
tellraw @a[scores={st=4}] {"text":"<貝露> 哥哥沒問題嗎？是不是遇到困難了？","clickEvent":{"action":"run_command","value":"/function dialog:dia/add"}}
tellraw @a[scores={st=5}] {"text":"<亞加> 我沒問題的！我只是不太擅長剪這些人造物而已——成功了！","clickEvent":{"action":"run_command","value":"/function dialog:dia/add"}}
execute if entity @a[tag=player,scores={st=5}] run function dialog:dia/ch1_1/func/destroy_ironbar
tellraw @a[scores={st=6}] {"text":"<格藍> 哈，區區鐵柵欄也想擋住我們的任務，門都沒有！","clickEvent":{"action":"run_command","value":"/function dialog:dia/add"}}
