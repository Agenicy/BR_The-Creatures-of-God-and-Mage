...
tellraw @a[tag=player,scores={st=2,check_success=1}] {"text":"→〔成功〕劈開鐵柵欄了！","color":"aqua","clickEvent":{"action":"run_command","value":"/execute store success score @s dia_st run trigger dia_choose set 24"}}
tellraw @a[tag=player,scores={st=2,check_success=0}] {"text":"→〔失敗〕〔精神／普通〕失手了！","color":"aqua","clickEvent":{"action":"run_command","value":"/execute store success score @s dia_st run trigger dia_choose set 9"}}
