...
tellraw @a[tag=player,scores={st=2,check_success=1}] {"text":"→〔成功〕赫斯克被吸引過去了！","color":"aqua","clickEvent":{"action":"run_command","value":"/execute store success score @s dia_st run trigger dia_choose set 36"}}
tellraw @a[tag=player,scores={st=2,check_success=0}] {"text":"→〔失敗〕好像沒什麼效果？","color":"aqua","clickEvent":{"action":"run_command","value":"/execute store success score @s dia_st run trigger dia_choose set 37"}}
