...
tellraw @a[scores={st=6}] {"text":"<格藍> 不行啦……我力量還沒恢復……就算恢復了也只能帶一個人走……","clickEvent":{"action":"run_command","value":"/function a:dia/add"}}
tellraw @a[scores={st=7}] {"text":"<卡司> 大家注意，不要反射性地拔出武器來啊，會被天眼盯上的。","clickEvent":{"action":"run_command","value":"/function a:dia/add"}}
