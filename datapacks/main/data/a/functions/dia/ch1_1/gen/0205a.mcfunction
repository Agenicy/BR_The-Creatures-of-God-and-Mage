...
tellraw @a[scores={st=2,check_success=0}] {"text":"→［成功］他們完全沒發現我們呢，看來是安全了。","color":"aqua","clickEvent":{"action":"run_command","value":"/execute store success score @s dia_st run trigger dia_choose set 44"}}
tellraw @a[scores={st=2,check_success=}] {"text":"→［失敗］他們怎麼轉過來了！難道是呼吸太大聲了嗎？","color":"aqua","clickEvent":{"action":"run_command","value":"/execute store success score @s dia_st run trigger dia_choose set "}}
