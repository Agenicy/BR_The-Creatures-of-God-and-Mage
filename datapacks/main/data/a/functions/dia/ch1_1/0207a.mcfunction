...
tellraw @a[tag=player,scores={st=2,check_success=1}] {"text":"→〔成功〕通過街道了！果然先跑是正確的！","color":"aqua","clickEvent":{"action":"run_command","value":"/execute store success score @s dia_st run trigger dia_choose set 46"}}
tellraw @a[tag=player,scores={st=2,check_success=0}] {"text":"→〔失敗〕可惡、被追上了！","color":"aqua","clickEvent":{"action":"run_command","value":"/execute store success score @s dia_st run trigger dia_choose set 44"}}
