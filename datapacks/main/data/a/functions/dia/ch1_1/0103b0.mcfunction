...
tellraw @a[tag=player,scores={st=2,check_success=1}] {"text":"→〔成功〕原來我身手還不賴嘛。","color":"aqua","clickEvent":{"action":"run_command","value":"/execute store success score @s dia_st run trigger dia_choose set 18"}}
tellraw @a[tag=player,scores={st=2,check_success=0}] {"text":"→〔失敗〕〔幸運／普通〕翻太慢了！希望沒有被看到……","color":"aqua","clickEvent":{"action":"run_command","value":"/execute store success score @s dia_st run trigger dia_choose set 19"}}
