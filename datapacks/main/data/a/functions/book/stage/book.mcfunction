tellraw @a {"text":"\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n------","color":"gold","extra":[{"text":"☆神奇的魔法書庫☆","color":"yellow"},{"text":"------","color":"gold"}]}
tellraw @a {"text":"目前分頁: ","color":"gold","extra":[{"text":"其他文件","color":"aqua"},{"text":"  [回首頁]","color":"dark_green","hoverEvent":{"action":"show_text","value":"點選回首頁"},"clickEvent":{"action":"run_command","value":"/function a:book/book"}}]}

tellraw @a {"text":"● 子目錄: 遊戲說明","color":"green","hoverEvent":{"action":"show_text","value":"關於本遊戲的說明……誒?"},"clickEvent":{"action":"run_command","value":"/function a:book/stage/sys/gen/book"}}
tellraw @a {"text":"● 子目錄: 天眼大門","color":"green","hoverEvent":{"action":"show_text","value":"在解開天眼大門陣法時取得的文件"},"clickEvent":{"action":"run_command","value":"/function a:book/stage/door_skyeye/gen/book"}}

tellraw @a {"text":"------------------------------","color":"gold"}

function a:book/book_bgs