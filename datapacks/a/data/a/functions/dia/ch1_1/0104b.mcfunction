...
tellraw @a[tag=player,scores={st=2,check_success=1}] {"text":"→〔成功〕呼……我們做到了！","color":"aqua","clickEvent":{"action":"run_command","value":"/execute store success score @s dia_st run trigger dia_choose set 45"}}
tellraw @a[tag=player,scores={st=2,check_success=0}] {"text":"→〔失敗〕可惡！就差這麼一點——","color":"aqua","clickEvent":{"action":"run_command","value":"/execute store success score @s dia_st run trigger dia_choose set 44"}}
