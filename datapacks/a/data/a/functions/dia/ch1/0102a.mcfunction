...
tellraw @a[tag=player,scores={st=2,check_success=1}] {"text":"→〔成功〕哼，小意思。","clickEvent":{"action":"run_command","value":"/execute store success score @s dia_st run trigger dia_choose set 24"}}24
tellraw @a[tag=player,scores={st=2,check_success=0}] {"text":"→〔失敗〕〔靈巧／普通〕糟糕，手滑了！","clickEvent":{"action":"run_command","value":"/execute store success score @s dia_st run trigger dia_choose set 6"}}6
