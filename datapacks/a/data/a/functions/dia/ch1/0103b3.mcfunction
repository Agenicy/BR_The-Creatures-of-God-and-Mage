...
tellraw @a[tag=player,scores={st=3}] {"text":"<卡司> 呃、你們什麼時候翻到那種地方的？","clickEvent":{"action":"run_command","value":"/function a:dia/add"}}
...
tellraw @a[tag=player,scores={st=6}] {"text":"→〔目標達成〕通過小巷子","clickEvent":{"action":"run_command","value":"/execute store success score @s dia_st run trigger dia_choose set 24"}}24
