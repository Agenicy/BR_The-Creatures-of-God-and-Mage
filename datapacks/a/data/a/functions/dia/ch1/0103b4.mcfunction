...
execute if entity @a[tag=player,scores={st=4}] run function a:ch1/destroy_ironbar
tellraw @a[tag=player,scores={st=5}] {"text":"<亞加> 呃呃……抱歉……","clickEvent":{"action":"run_command","value":"/function a:dia/add"}}
tellraw @a[tag=player,scores={st=5}] {"text":"→〔目標失敗〕倉皇逃跑","clickEvent":{"action":"run_command","value":"/execute store success score @s dia_st run trigger dia_choose set 22"}}22
